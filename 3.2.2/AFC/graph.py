...
if len(csv_files) < -1:
    ...
    for i in range(2):  # Только два первых файла
        ...
        x_label    = "f, кГц"
        xerr_label = "\sigma_f, кГц"
        y_label    = "U_c, В"
        yerr_label = "\sigma{U_c}, В"
    ...
    plt.gca().xaxis.set_major_locator(MultipleLocator(1))   # Основные метки через ... единицу по оси X
    plt.gca().xaxis.set_minor_locator(AutoMinorLocator(4))  # Дополнительные метки (... на каждый интервал)

    plt.gca().yaxis.set_major_locator(MultipleLocator(0.2)) # Основные метки через ... единиц по оси Y
