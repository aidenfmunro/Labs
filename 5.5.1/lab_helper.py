...
def propogate_uncertainty(val, arr):
    sigma = 0
    for i in arr:
        sigma += (i[1] / i[0]) ** 2
...
def get_fit_line_points(x, y, dx, dy, k_b):
    minx = min(x) - max(dx)
    miny = min(y) - max(dy)
    maxx = max(x) + max(dx)
    maxy = max(y) + max(dy)
...
def plot_data(file, title, xlabel, ylabel, x, y, dx, dy):
...
def plot_data_with_fit_and_table(file, title, xlabel, ylabel, x, y, dx, dy, k_b, table_x, table_y):
    ...
    plt.plot(x_lin, [i * k_b[0][0] + k_b[1][0] for i in x_lin], color="cyan", label = "Аппроксимация")
...
def fit_linear_and_plot_data(file, title, xlabel, ylabel, x, y, dx, dy):

    k, b = fit_linear(x, y)

    plot_data_with_fit_and_table(file, title, xlabel, ylabel, x, y, dx, dy, (k, b), 0, 0)

    return k, b

def fit_linear_and_plot_data_with_table(file, title, xlabel, ylabel, x, y, dx, dy, table_x, table_y):

    k, b = fit_linear(x, y)

    plot_data_with_fit_and_table(file, title, xlabel, ylabel, x, y, dx, dy, (k, b), table_x, table_y)

    return k, b

def plot_and_fit_proportional(file, title, xlabel, ylabel, x, y, dx, dy):
